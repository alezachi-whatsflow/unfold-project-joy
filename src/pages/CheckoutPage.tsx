...
function OrderSummary({ state, price, onContinue }: { state: CheckoutState; price: PricePreview; onContinue: () => void }) {
  ...
  return (
    <div className="border border-border bg-card p-6 space-y-4 sticky top-6">
      ...
      <button onClick={onContinue} className="w-full py-3 font-bold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "var(--co-primary)" }}>
        Continuar →
      </button>
    </div>
  );
    ...
    <div className="min-h-screen bg-background text-foreground" style={styleVars}>
      ...
      <header className="border-b border-border px-6 py-4 flex items-center gap-3">
        {wlBranding?.logo_url
          ...
          : <div className="h-8 px-3 py-1 font-black text-sm text-white" style={{ backgroundColor: primaryColor }}>{appName}</div>
        }
              ...
              {(["solo_pro", "profissional"] as Plan[]).map((p) => {
                ...
                return (
                  <button key={p} onClick={() => { setState(s => ({ ...s, plan: p })); setStep(1); }}
                    className={`relative border-2 p-8 text-left transition-all hover:${isSelected ? "border-[var(--co-primary)] bg-[var(--co-primary)]/5" : "border-border hover:border-[var(--co-primary)]/50"}`}>
                    {isRecommended && (
                    ...
                    <div className="mt-6 py-3 font-bold text-white text-center text-sm" style={{ backgroundColor: primaryColor }}>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {(["none", "basico", "intermediario", "avancado"] as FacilitePlan[]).map(f => (
                    <button key={f} onClick={() => setState(s => ({ ...s, facilitePlan: f }))}
                      className={`border-2 p-3 text-center transition-all ${state.facilitePlan === f ? "border-[var(--co-primary)] bg-[var(--co-primary)]/10" : "border-border hover:border-[var(--co-primary)]/50"}`}>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep(0)} className="flex items-center gap-2 px-4 py-2 border border-border text-sm hover:bg-accent transition-colors">
        ...
        {step === 2 && (
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="md:col-span-2 space-y-1">
                  ...
                  <input value={companyName} onChange={e => setCompanyName(e.target.value)} placeholder="Minha Empresa Ltda"
                    className="w-full bg-background border border-border px-4 py-3 text-sm outline-none focus:ring-2 ring-[var(--co-primary)] transition" />
                </div>

                <div className="md:col-span-2 space-y-1">
                  <label className="text-sm font-medium">Endereço da conta *</label>
                  <div className="flex border border-border overflow-hidden focus-within:ring-2 ring-[var(--co-primary)]">
              ...
              <div className="flex gap-3">
                {!sessionId && (
                  <button onClick={() => setStep(1)} className="flex items-center gap-2 px-4 py-2 border border-border text-sm hover:bg-accent transition-colors">
                    <ChevronLeft className="h-4 w-4" /> Voltar
                  </button>
                )}
              </div>
        ...
        {step === 3 && (
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex gap-2 p-1 bg-secondary w-fit">
                {(["pix", "boleto", "cartao"] as const).map(t => (
                  <button key={t} onClick={() => setPayTab(t)}
                    className={`px-5 py-2 text-sm font-bold transition-all ${payTab === t ? "bg-background shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                ...
                <div className="border border-border p-6 space-y-5">
                  {!pixCode ? (
                    <button onClick={handleCreatePayment} disabled={isCreatingSession}
                      className="w-full py-4 font-bold text-white text-lg flex items-center justify-center gap-3 hover:opacity-90 transition-opacity disabled:opacity-60"
                      style={{ backgroundColor: primaryColor }}>
                      {isCreatingSession ? <><Loader2 className="h-5 w-5 animate-spin" /> Gerando PIX...</> : "Gerar QR Code PIX"}
                    </button>
                  ) : (
                    <div className="space-y-4">
                      <div className="bg-white p-6 mx-auto w-48 h-48 flex items-center justify-center">
                        <div className="text-6xl font-black text-black text-center">📷<br /><span className="text-sm font-normal">QR Code</span></div>
                      </div>
                      <div className="flex gap-2">
                        <input readOnly value={pixCode} className="flex-1 bg-secondary px-3 py-2 text-xs font-mono text-muted-foreground overflow-hidden" />
                        <button onClick={copyPix} className="px-4 py-2 bg-secondary hover:bg-accent transition-colors flex items-center gap-2 text-sm font-medium">
              ...
              {/* BOLETO */}
              {payTab === "boleto" && (
                <div className="border border-border p-6 space-y-4 text-center">
                  ...
                  <button onClick={handleCreatePayment} disabled={isCreatingSession}
                    className="w-full py-3 font-bold text-white hover:opacity-90 transition-opacity"
                    style={{ backgroundColor: primaryColor }}>
                ...
                <div className="border border-border p-6 space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="md:col-span-2 space-y-1">
                      ...
                      <input placeholder="0000 0000 0000 0000" className="w-full bg-background border border-border px-4 py-3 text-sm outline-none focus:ring-2 ring-[var(--co-primary)] font-mono" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm font-medium">Nome no cartão</label>
                      <input placeholder="JOAO DA SILVA" className="w-full bg-background border border-border px-4 py-3 text-sm outline-none focus:ring-2 ring-[var(--co-primary)]" />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-sm font-medium">Validade</label>
                        <input placeholder="MM/AA" className="w-full bg-background border border-border px-4 py-3 text-sm outline-none focus:ring-2 ring-[var(--co-primary)] font-mono" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-sm font-medium">CVV</label>
                        <input placeholder="123" type="password" className="w-full bg-background border border-border px-4 py-3 text-sm outline-none focus:ring-2 ring-[var(--co-primary)] font-mono" />
                      </div>
                    </div>
                  </div>
                  <button onClick={handleCreatePayment} disabled={isCreatingSession}
                    className="w-full py-3 font-bold text-white text-sm hover:opacity-90 transition-opacity"
                    style={{ backgroundColor: primaryColor }}>
              ...
              <button onClick={() => setStep(2)} className="flex items-center gap-2 px-4 py-2 border border-border text-sm hover:bg-accent transition-colors">
                <ChevronLeft className="h-4 w-4" /> Voltar
              </button>
            </div>
