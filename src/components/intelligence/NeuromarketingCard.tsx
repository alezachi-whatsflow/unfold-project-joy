        ...
        <CardContent>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {Object.entries(a.gatilhos_cialdini).map(([key, trigger]) => {
              ...
              return (
                <div key={key} className={cn("rounded-lg border p-3 space-y-1", trigger.presente ? "border-primary/20 bg-primary/5" : "border-border bg-secondary/30")}>
...
function BrainCard({ icon: Icon, title, color, score, desc, gaps }: {
  icon: React.ElementType; title: string; color: string; score: number; desc: string; gaps: string[];
}) {
  return (
    <Card className="border-border bg-card">
