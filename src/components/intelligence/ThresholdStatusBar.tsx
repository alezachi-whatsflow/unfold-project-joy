import { Globe, Instagram, MapPin, Shield } from "lucide-react";
...
export function ThresholdStatusBar({ overall, website, instagram, gmn }: ThresholdStatusBarProps) {
  const channels = [
        ...
        <div className="grid grid-cols-3 gap-2">
          {channels.map(({ label, icon: Icon, threshold }) => (
            <div key={label} className="flex flex-col items-center gap-1 rounded-lg bg-secondary/50 p-2">
              ...
              <Badge
                variant="outline"
                className={cn("text-[9px]", getStatusColor(threshold!.status))}
              >
                {threshold!.label}
              </Badge>
            </div>
