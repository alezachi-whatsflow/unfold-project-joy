...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        "flex flex-col h-screen shrink-0 border-r border-sidebar-border bg-sidebar overflow-hidden",
        sidebarWidth
      )}
              ...
              {group.items.map((item) => (
                <li key={item.title}>
                  <RouterNavLink
                    ...
                    style={{ padding: collapsed ? "8px 0" : "7px 10px" }}
                    className={({ isActive }) =>
                      cn(
                        menuItemBase,
                  ...
                  >
                    <span className="relative shrink-0 flex items-center justify-center">
                      ...
                      {/* Collapsed dot badge */}
                      {"badgeKey" in item && item.badgeKey === "overdue" && collapsed && overdueCount && overdueCount > 0 && (
                        <span
